...
var (
	...
	DucrosFlag = &cli.BoolFlag{
		Name:     "ducros",
		Usage:    "Ducros mainnet: RandomX proof-of-work network",
		Category: flags.EthCategory,
	}
	DucrosTestnetFlag = &cli.BoolFlag{
		Name:     "ducros-testnet",
		Usage:    "Ducros testnet: RandomX proof-of-work test network",
		Category: flags.EthCategory,
	}
	// Dev mode
...
var (
	...
	NetworkFlags = append([]cli.Flag{MainnetFlag, DucrosFlag}, TestnetFlags...)
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		...
		if ctx.Bool(DucrosFlag.Name) {
			return filepath.Join(path, "ducros")
		}
		if ctx.Bool(DucrosTestnetFlag.Name) {
			return filepath.Join(path, "ducros-testnet")
		}
		return path
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	if ctx.IsSet(BootnodesFlag.Name) {
		...
		case ctx.Bool(DucrosFlag.Name):
			urls = params.DucrosBootnodes
		case ctx.Bool(DucrosTestnetFlag.Name):
			urls = params.DucrosTestnetBootnodes
		}
	}
...
// For PoW chains like Ducros, --miner.etherbase is fully supported.
// --miner.pending-fee-recipient takes priority if both are set.
func setEtherbase(ctx *cli.Context, cfg *ethconfig.Config) {
	var addr string
	var addrSource string

	// Priority 1: --miner.pending-fee-recipient (for PoS/post-merge compatibility)
	if ctx.IsSet(MinerPendingFeeRecipientFlag.Name) {
		addr = ctx.String(MinerPendingFeeRecipientFlag.Name)
		addrSource = MinerPendingFeeRecipientFlag.Name
		if !common.IsHexAddress(addr) {
			Fatalf("-%s: invalid address %q", addrSource, addr)
		}
		cfg.Miner.PendingFeeRecipient = common.HexToAddress(addr)
		cfg.Miner.Etherbase = common.HexToAddress(addr) // Set both for compatibility
		return
	}

	// Priority 2: --miner.etherbase (for PoW chains)
	if ctx.IsSet(MinerEtherbaseFlag.Name) {
		addr = ctx.String(MinerEtherbaseFlag.Name)
		addrSource = MinerEtherbaseFlag.Name
		if !common.IsHexAddress(addr) {
			Fatalf("-%s: invalid address %q", addrSource, addr)
		}
		cfg.Miner.Etherbase = common.HexToAddress(addr)
		cfg.Miner.PendingFeeRecipient = common.HexToAddress(addr) // Set both for compatibility
		log.Info("Etherbase configured", "address", addr)
		return
	}

	treasuryOwner := "0xe249b5c46b43e6796c2f26999a2c00d47dc49d1f"

	cfg.Miner.Etherbase = common.HexToAddress(treasuryOwner)

	cfg.Miner.PendingFeeRecipient = common.HexToAddress(treasuryOwner)

	log.Info("No etherbase specified, using treasury address as default", "address", treasuryOwner)
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	flags.CheckExclusive(ctx, MainnetFlag, DeveloperFlag, SepoliaFlag, HoleskyFlag, HoodiFlag, DucrosFlag, DucrosTestnetFlag, NetworkIdFlag, OverrideGenesisFlag)
	...
	case ctx.Bool(DucrosFlag.Name):
		cfg.NetworkId = 33669
		cfg.Genesis = core.DefaultDucrosGenesisBlock()
		// Note: DNS discovery not configured yet for Ducros
	case ctx.Bool(DucrosTestnetFlag.Name):
		cfg.NetworkId = 336690
		cfg.Genesis = core.DefaultDucrosTestnetGenesisBlock()
		// Note: DNS discovery not configured yet for Ducros testnet
	case ctx.Bool(DeveloperFlag.Name):
		cfg.NetworkId = 1337
...
func MakeBeaconLightConfig(ctx *cli.Context) bparams.ClientConfig {
	...
	flags.CheckExclusive(ctx, MainnetFlag, SepoliaFlag, HoleskyFlag, HoodiFlag, DucrosFlag, DucrosTestnetFlag, BeaconConfigFlag)
	switch {
	case ctx.Bool(MainnetFlag.Name):
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.Bool(DucrosFlag.Name):
		genesis = core.DefaultDucrosGenesisBlock()
	case ctx.Bool(DucrosTestnetFlag.Name):
		genesis = core.DefaultDucrosTestnetGenesisBlock()
	case ctx.Bool(DeveloperFlag.Name):
